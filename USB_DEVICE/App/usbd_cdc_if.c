...
int32_t trcCDCReceive(uint8_t *data)
{
	...
	{
		if (commandBuffer.idx>=58) // more than what is stored, number of bytes will be .idx
		{
			//memcpy(data,commandBuffer.data, commandBuffer.idx);
			memcpy(data,commandBuffer.data, 58);
...
int32_t SendUSBData(uint8_t *data, uint32_t size )
{
	int32_t result;
...
int32_t trcCDCTransmitStr(uint8_t *data )
{
	int32_t result;
